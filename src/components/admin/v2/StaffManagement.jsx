  ...
  const handleSave = async (staffData) => {
    try {
      ...
      try {
        ...
        showToast("Error fetching IP:" + error.message, "error");
      }

      const dataToSend = { ...staffData, ip };

      if (editingStaff) {
        await axiosInstance.put(`/staff/${editingStaff._id}`, dataToSend);
      } else {
        await axiosInstance.post('/staff', dataToSend);
      }
      fetchStaff();
      setShowForm(false);
